...
<table class="form-table fancy-table" style="clear:none;">
	<tbody>
		...
		<tr valign="top">
			...
			<td>
				<fieldset>
					<input type="checkbox" class="onoffswitch-checkbox" name="mfbfw[zoomOpacity]" id="zoomOpacity"<?php if ( isset( $settings[ 'zoomOpacity' ] ) && $settings[ 'zoomOpacity' ] ) echo ' checked="yes"'; ?> />					
		...
		<tr valign="top">
			...
			<td>
				<fieldset>
					<input type="checkbox"  class="onoffswitch-checkbox" name="mfbfw[easing]" id="easing"<?php if ( isset( $settings[ 'easing' ] ) && $settings[ 'easing' ] ) echo ' checked="yes"'; ?> />
					<label for="easing" class="onoffswitch-label"></label>
					<?php _e( 'Activate easing (default: off)', 'mfbfw' ); ?><br />
						...
						<label for="easingOut">
							<select name="mfbfw[easingOut]" id="easingOut">
								...
								foreach ( $easingArray as $key => $easingOut ) {
									if ( $settings[ 'easingOut' ] != $easingOut )
										$selected	 = '';
									else
										$selected	 = ' selected';
						...
						<label for="easingChange">
							<select name="mfbfw[easingChange]" id="easingChange">
								...
								foreach ( $easingArray as $key => $easingChange ) {
									if ( isset( $settings[ 'easingChange' ] ) && $settings[ 'easingChange' ] != $easingChange )
										$selected	 = '';
									else
										$selected	 = ' selected';
